...
.venv/bin/python examples/demo_assess.py        # also demo_adjudicate / demo_compare / demo_goalie / demo_micro / demo_edge
...
1. **Boundary — `src/schemas.py`.** Strict pydantic card schemas (`SkaterCard`, `DefenseCard`, `GoalieCard`, plus `ForwardMicroCard` / `DefenseMicroCard` for the $10-tier microstat card), `extra="forbid"`, percentiles bounded 0–100. A mis-extracted card fails loudly here rather than producing a wrong answer. Micro cards require an explicit `card_kind: "micro"` discriminator and a `season`; `_parse_card` refuses a micro card with no `position` (the pool comes from the card footer, never a silent default). NHL Edge pages (`GoalieEdgeCard` / `SkaterEdgeCard`, discriminator `card_kind: "edge"`, required `season` + `gp`) model each legend-table row as an `EdgeMetric {value, avg?, percentile?}` with percentile bounded 50–100: `None` IS the site's "<50th" bucket, so a sub-50 number is structurally impossible to fabricate. `_parse_card` refuses an Edge page as a primary card — it enters only through `assess_player`'s `edge_card` param, validated against the schema the primary card's pool selects.

2. **Engine — `src/engine/`.** Pure transforms over a validated card:
   - `assess.py` — `assess_player(card, config, micro_card, edge_card)` → `Assessment` (skater), `GoalieAssessment`, or `MicroAssessment` (microstat card). Dispatches on card type. With both cards for one player (standard `card` + `micro_card`) the `Assessment` gains an articulation-only `micro_insights` synthesis (season-vs-projection divergences ≥ `DIVERGENCE_MIN`, tracked evidence behind the verdicts) — the tier never moves. An `edge_card` (same player, same pool) adds an articulation-only `edge_vetting` cross-check to any of the three shapes; the vetting logic lives in `edge.py` (`vet_edge`).
...
- `config/interpretation.yaml` — tier bands, the auto-attached caveats (including the micro caveats: `micro_single_season`, `micro_unadjusted`, `micro_style_not_value`, and `defense_repeatability` for forward EV-defense strengths; plus the Edge caveats: `edge_single_season`, `edge_different_source`, `edge_counts_workload`, `edge_tools_not_value`), position rules, goalie reading rules, `war_reading` (the ~37th-percentile replacement anchor and point-estimate framing, distilled from the model author's methodology write-ups — articulation only, never moves a tier), `micro_rules` (the cross-regime WAR-row note), `micro_profiles` (the paired style reads: shot selectivity, passing quality, attack style, D rush defense — thresholds and wording), `edge_rules` (the Edge vetting thresholds: `save_pct_gap`, `zone_time_gap`, and the articulation-only note), and the claim→metric `dimensions` dictionary (with aliases; micro dimensions use `applies_to: micro`). Tune behavior by editing YAML, not code.
- `config/glossary.yaml` — one entry per card metric (skater + goalie + microstat + the NHL Edge terms): plain-language `definition` + the single key interpretive `caveat`, plus `aliases` for `explain_metric` lookup. Six micro entries are inferred — the site publishes no official definitions for those boxes (checked 2026-07-18), so the inferred reads are canonical; their caveats disclose the inference (test-guarded; see DECISIONS.md). A few documented-but-unsampled Edge boxes (avg shot speed, zone starts, speed bursts, differentials, 5v5 save% splits) are glossary-only: defined for `explain_metric`, no schema field until a real sample surfaces.
...
- **No em dashes, anywhere in output.** User rule (2026-07-19): tool answers and PDFs must never contain an em dash. All source text (config, engine strings, templates) is written with plain hyphens instead; `engine.common.strip_em_dashes` backstops the server results and the rendered report HTML at runtime; tool descriptions steer the host's own prose the same way. `tests/test_no_em_dashes.py` scans the repo and the outputs, so a reintroduced em dash fails the suite. Write new strings with hyphens, commas, or colons.
- **Team affiliation is never surfaced.** The logo/team on a card goes stale with trades, so it is noise, not context: card schemas accept `team` for compatibility but no engine output or report carries it, and the `assess_player` description tells the host to ignore the logo entirely (guarded in `test_server.py`). Do not reintroduce a team read anywhere.
...
- **NHL Edge is supplemental and articulation-only.** `edge_card` only ever rides alongside the card being assessed — never assessed alone, never in `adjudicate_claim` or `compare_players` (a deliberate scope decision, see DECISIONS.md 2026-07-21). The signal hierarchy: rates with a printed comparison average drive corroborate/contradict calls (thresholds in `edge_rules`); a rate's exact percentile is color, and "<50th" is direction-only; raw counts NEVER drive a call — they accumulate with games played (the Vanecek finding), so they stay descriptive with GP named and count percentiles are never cited. Tools (hardest shot, max speed, miles skated) are style color with `edge_tools_not_value` (the Makar/Miller finding); zone-time calls reuse the `deployment_not_value` caveat — one caveat, one home, deliberately NOT merged with the tools caveat.
